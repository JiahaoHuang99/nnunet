...
class nnUNetClsTrainer_Lambda0p05(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.configuration_manager.network_arch_init_kwargs['classification_classes'] = self.classification_classes
        self.label_patho = 7

        self.classification_lambda = 0.05
        ...
        self.loss_seg = None  # -> self.initialize
        self.loss_cls = None  # -> self.initialize
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # TODO: ADD MORE LOSS
            self.loss_seg = self._build_loss()
            self.loss_cls = self._build_loss_cls()
    ...
    def _build_loss_cls(self):
        return nn.CrossEntropyLoss()

    def prepare_target_cls(self, keys, device):
        ...
        for i in range(batch_size):
            if "quiz_0" in keys[i]:
                cls[i] = 0
            elif "quiz_1" in keys[i]:
                cls[i] = 1
            elif "quiz_2" in keys[i]:
                cls[i] = 2
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        target_seg = target
        target_cls = self.prepare_target_cls(keys, device=data.device)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l_seg = self.loss_seg(output_seg, target_seg)
            l_cls = self.loss_cls(output_cls, target_cls)
            l = l_seg + self.classification_lambda * l_cls
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                self.print_to_log_file(f"Epoch {epoch}, step {batch_id}")
                train_outputs.append(self.train_step(next(self.dataloader_train)))
            self.on_train_epoch_end(train_outputs)

            with torch.no_grad():
                self.print_to_log_file(f"Epoch {epoch}, validation start!")
                self.on_validation_epoch_start()
                val_outputs = []
                for batch_id in range(self.num_val_iterations_per_epoch):
                    val_outputs.append(self.validation_step(next(self.dataloader_val)))
                self.on_validation_epoch_end(val_outputs)
                self.print_to_log_file(f"Epoch {epoch}, validation done!")
